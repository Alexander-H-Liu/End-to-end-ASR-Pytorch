...
class BeamDecoder(nn.Module):
    ...
    def forward(self, audio_feature, feature_len):
        ...
        final_hypothesis, next_top_hypothesis = [], []                                # Cache of beam search
        ctc_state, ctc_prob, candidates, lm_state = None, None, None, None            # Incase ctc is disabled
        ...
        for t in range(max_output_len):
            for hypothesis in prev_top_hypothesis:
                ...
                if self.apply_ctc:
                    ...
                    _, ctc_candidates = cur_prob.topk(ctc_beam_size,dim=-1)
                    candidates = ctc_candidates[0,0].cpu().tolist()
                    ...
                    for idx,char in enumerate(candidates):
                        hack_ctc_char[0,0,char] = ctc_char[idx]
                    cur_prob = (1-self.ctc_w)*cur_prob + self.ctc_w*hack_ctc_char#ctc_char#
                    cur_prob[0,0,0] = LOG_ZERO # Hack to ignore <sos>

                # Joint RNN-LM decoding
                if self.apply_lm:
                    lm_input = prev_token.unsqueeze(0) # assuming batch size always 1
                    lm_output, lm_state = self.lm(lm_input,torch.ones([batch_size]),hidden=prev_lm_state)
                    lm_output = lm_output.squeeze(0)   # assuming batch size always 1
                    cur_prob += self.lm_w*F.log_softmax(lm_output,dim=-1)

                # Beam search
                topv, topi = cur_prob.squeeze(1).topk(self.beam_size) # Note: Ignored batch dim.
...
class Hypothesis:
    ...
    def __init__(self, decoder_state, output_seq, output_scores, lm_state, ctc_state, ctc_prob, att_map):
        ...
        if type(lm_state) is tuple:
            self.lm_state = (lm_state[0].cpu(),lm_state[1].cpu()) # LSTM state
    ...
    def get_state(self, device):
        ...
        if type(self.lm_state) is tuple:
            lm_state = (self.lm_state[0].to(device),self.lm_state[1].to(device)) # LSTM state
        elif self.lm_state is None:
            lm_state = None                                  # Init state
        else:
            lm_state = self.lm_state.to(device)                        # GRU state
