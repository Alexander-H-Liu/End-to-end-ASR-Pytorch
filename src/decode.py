    ...
    def forward(self, audio_feature, feature_len):
        ...
        for t in range(max_output_len):
            for hypothesis in prev_top_hypothesis:
                ...
                if self.apply_lm:
                    lm_input = prev_token.unsqueeze(1) # assuming batch size always 1, resulting 1x1
                    lm_output, lm_state = self.lm(lm_input,torch.ones([batch_size]),hidden=prev_lm_state)
                    lm_output = lm_output.squeeze(0)   # assuming batch size always 1,  resulting 1xV
                    cur_prob += self.lm_w*lm_output.log_softmax(dim=-1)

                # Beam search
                topv, topi = cur_prob.squeeze(0).topk(self.beam_size) # Note: Ignored batch dim.
